...
def run_base_model_dcn(dfTrain, dfTest, folds, dcn_params):
    ...
    func = lambda x, l: [x[i] for i in l]
    for i, (train_idx, valid_idx) in enumerate(folds):
        cate_Xi_train_, cate_Xv_train_, numeric_Xv_train_, y_train_ = func(cate_Xi_train, train_idx), func(
            cate_Xv_train, train_idx), func(numeric_Xv_train, train_idx), func(y_train, train_idx)
        cate_Xi_valid_, cate_Xv_valid_, numeric_Xv_valid_, y_valid_ = func(cate_Xi_train, valid_idx), func(
            cate_Xv_train, valid_idx), func(numeric_Xv_train, valid_idx), func(y_train, valid_idx)
...
if __name__ == "__main__":
    ...
    folds = list(
        StratifiedKFold(n_splits=config.NUM_SPLITS, shuffle=True, random_state=config.RANDOM_SEED).split(X_train, y_train))
