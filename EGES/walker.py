...
class RandomWalker:
    ...
    def preprocess_transition_probs(self):
        ...
        alias_nodes = {}
        for node in G.nodes():
            unnormalized_probs = [G[node][nbr].get('weight', 1.0) for nbr in G.neighbors(node)]  #保存start的邻居节点的权重
            norm_const = sum(unnormalized_probs)
