...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, feature_size, field_size, embedding_size=8, dropout_fm=[1.0,1.0], deep_layers=[32,32],
                 dropout_deep=[0.5,0.5,0.5], deep_layer_activation=tf.nn.relu, epoch=10, batch_size=256, learning_rate=0.001,
                 optimizer="adm", batch_norm=0, batch_norm_decay=0.995, verbose=False, random_seed=2016, use_fm=True,
                 use_deep=True, loss_type="logloss", eval_metric=roc_auc_score, l2_reg=0.0, greater_is_better=True):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            # square-sum-part
            self.squared_feature_emb = tf.square(self.embeddings)
            self.squared_sum_feature_emb = tf.reduce_sum(self.squared_feature_emb, 1)

            # second order
            self.y_second_order = 0.5 * tf.subtract(self.summed_features_emb_square, self.squared_sum_feature_emb)
    ...
    def _initialize_weights(self):
        ...
        weights['feature_bias'] = tf.Variable(tf.random_normal([self.embedding_size, 1], 0.0, 1.0), name="feature_bias")
