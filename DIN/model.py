...
from DIN.modules import *


class DIN(Model):

    def __init__(self, feature_columns, behavior_feature_list, att_hidden_units=(80, 40), ffn_hidden_units=(80, 40), att_activation='prelu', ffn_activation='prelu', maxlen=40, dnn_dropout=0., embed_reg=1e-4):
        """
        DIN
        ...
        self.embed_sparse_layers = [
            ...
            if feat['feat_name'] not in behavior_feature_list
        ]

        # behavior embedding layers, item id and category id
        self.embed_seq_layers = [
            Embedding(
                input_dim=feat['feat_num'],
                input_length=1,
                output_dim=feat['embed_dim'],
            ...
            for feat in self.sparse_feature_columns
            if feat['feat_name'] in behavior_feature_list
        ]
    ...
    def call(self, inputs):
        ...
        # seq, item embedding and category
        ...
        # att  dim = (None, d*2)
        user_info = self.attention_layer([item_embed, seq_embed, seq_embed, mask])

        # concat user_info(att hist), candidate item embedding, other features
        if self.embed_len > 0 or self.other_sparse_len > 0:
    ...
    def summary(self, line_length=None, positions=None, print_fn=None):
        dense_inputs = Input(shape=(self.dense_len, ), dtype=tf.int32)
        ...
        tf.keras.Model(inputs=[dense_inputs, sparse_inputs, seq_inputs, item_inputs], outputs=self.call([dense_inputs, sparse_inputs, seq_inputs, item_inputs])).summary()


def test_model():
    dense_features = [{"feat": "a"}, {"feat": "b"}]
    sparse_features = [
        {'feat_name': 'item_id', 'feat_num': 100, 'embed_dim': 8},
        {'feat_name': 'cate_id', 'feat_num': 100, 'embed_dim': 8},
        {'feat_name': 'adv_id', 'feat_num': 100, 'embed_dim': 8}
    ]
